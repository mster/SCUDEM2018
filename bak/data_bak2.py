...
t_max = 0.5
t_step = 0.001
correction_constant = int(t_max/t_step) * 1.00
...
color = 0
rgb = (0,0,0)
theta_pool = np.linspace(-np.pi/2, np.pi/2, 21)
for t in theta_pool:
	n = 0
	...
	x_plot = pos_cache_[:,0][:(len(time) - 0)]
	y_plot = pos_cache_[:,1][:(len(time) - 0)]
	ax.plot_wireframe(X=x_plot,Y=time,Z=y_plot, color=rgb)
	scale = 1/len(theta_pool)
	print(color)
	color += scale
	rgb = (color,0,1-color)
	t_cur = 0
